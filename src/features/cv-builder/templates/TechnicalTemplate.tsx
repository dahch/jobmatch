...
const styles = StyleSheet.create({
  ...
  contact: { fontSize: 8, color: "#555", marginBottom: 14, flexDirection: "row" as const, flexWrap: "wrap" as const },
  contactItem: { marginRight: 12 },
  section: { marginBottom: 10 },
  sectionTitle: { fontSize: 10, fontFamily: "Courier-Bold", backgroundColor: "#111", color: "#fff", padding: "3 6", marginBottom: 6 },
  jobRow: { flexDirection: "row" as const, justifyContent: "space-between" as const, marginBottom: 2 },
  ...
  skillTag: { fontSize: 7, backgroundColor: "#e5e7eb", padding: "2 5", borderRadius: 2, marginRight: 4, marginBottom: 4 },
      ...
      <Page size="A4" style={styles.page}>
          ...
          <View style={styles.section}>
            ...
            {cv.skills.map((cat, i) => (
              <View key={i} style={{ marginBottom: 4 }}>
                <Text style={{ fontSize: 8, fontFamily: "Courier-Bold", marginBottom: 2 }}>{cat.category}:</Text>
        ...
        {cv.projects && cv.projects.length > 0 && (
          <View style={styles.section}>
            ...
            {cv.projects.map((p, i) => (
              <View key={i} style={{ marginBottom: 4 }}>
                <Text style={{ fontSize: 8, fontFamily: "Courier-Bold" }}>{p.name}</Text>
                <Text style={{ fontSize: 7, color: "#666" }}>[{p.technologies.join(", ")}]</Text>
