 ...
 */
export function parseAIJsonResponse<T = Record<string, unknown>>(content: string): T {
  // 1. Direct parse
  ...
  if (mdMatch) {
    try { return JSON.parse(mdMatch[1]) as T; } catch { /* continue */ }
    try { return JSON.parse(repairTruncatedJSON(mdMatch[1])) as T; } catch { /* continue */ }
  }
  ...
  if (jsonStart !== -1 && jsonEnd > jsonStart) {
    ...
    try { return JSON.parse(slice) as T; } catch { /* continue */ }
    try { return JSON.parse(repairTruncatedJSON(slice)) as T; } catch { /* continue */ }
  }

  throw new Error("Failed to parse AI response. The model may have hit a token limit.");
}
